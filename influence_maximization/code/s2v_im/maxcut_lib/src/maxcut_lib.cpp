...
int Init(const int argc, const char** argv)
{
    ...
    //std::cerr << "num_env " <<  cfg::num_env << std::endl;

    for (int i = 0; i < cfg::num_env; ++i)
        Simulator::env_list[i] = new MaxcutEnv(cfg::max_n, cfg::ratio_seeds, cfg::prob);
    test_env = new MaxcutEnv(cfg::max_n, cfg::ratio_seeds, cfg::prob);
...
int PlayGame(const int n_traj, const double eps, const double ratio_s)
{
    Simulator::run_simulator(n_traj, eps, ratio_s);
    return 0;
}
...
double Test(const int gid, const double ratio_s = -1.)
{
    ...
    double v = 0;
    int new_action;
    while (!test_env->isTerminal(ratio_s))
    {
        Predict(g_list, states, list_pred);
        auto& scores = *(list_pred[0]);
        new_action = arg_max(test_env->graph->num_nodes, scores.data(), test_env->seed_set);
...
double TestNoStop(const int gid, const double ratio_s = -1.)
{
    std::vector< std::shared_ptr<Graph> > g_list(1);
    std::vector< std::vector<int>* > states(1);
...
double GetSol(const int gid, int* sol, double ratio_s)
{
    std::vector< std::shared_ptr<Graph> > g_list(1);
    std::vector< std::vector<int>* > states(1);
