...
if __name__ == '__main__':
    ...
    sys.stdout.flush()
    for i in range(10):
        api.lib.PlayGame(100, ctypes.c_double(1.0), ctypes.c_double(float(opt['ratio_seeds'])))
    api.TakeSnapshot()
    ...
    for iter in range(int(opt['max_iter'])):
        ...
        if iter % 10 == 0:
            api.lib.PlayGame(10, ctypes.c_double(eps), ctypes.c_double(float(opt['ratio_seeds'])))

        if iter % 300 == 0:
            frac = 0.0
            for idx in range(n_valid):
                frac += api.lib.TestNoStop(idx, ctypes.c_double(float(opt['ratio_seeds'])))
            print 'iter', iter, 'eps', eps, 'average influence: ', frac / n_valid
            sys.stdout.flush()
